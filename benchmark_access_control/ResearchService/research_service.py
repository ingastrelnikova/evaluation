...
def log_latency(start_time, end_time, operation):
    latency = (end_time - start_time) * 1000  # Convert to milliseconds
    with open('latencies.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([start_time, end_time, latency, operation])
...
def check_authorization():
    ...
    end_time = time.time()
    log_latency(start_time, end_time, "policy checking")
    sys.stdout.flush()
...
def get_data():
    ...
    try:
        ...
        conn.close()
        end_time = time.time()
        log_latency(start_time, end_time, "data retrieval")
        return jsonify(rows)
...
if __name__ == '__main__':
    ...
    with open('latencies.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["start_time", "end_time", "latency_ms", "operation"])
